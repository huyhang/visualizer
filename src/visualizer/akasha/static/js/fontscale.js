...
// the choice, like the theme toggle. The no-flash init in the template applies a
// saved value before paint; this only handles the button and labelling.

const KEY = "akasha-fontscale";
// Pre-rename key, still read so a saved size survives the rebrand.
const LEGACY_KEY = "kb-fontscale";
const LEVELS = [
  { px: 16, name: "Normal" },
  { px: 18, name: "Large" },
  { px: 20, name: "Larger" },
...
function saved() {
  const px = parseInt(localStorage.getItem(KEY) || localStorage.getItem(LEGACY_KEY) || "", 10);
