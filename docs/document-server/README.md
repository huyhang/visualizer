...
This service's tests live in `tests/document_server/`:

```bash
pytest -q tests/document_server     # in-memory MongoDB (mongomock); no server needed
ruff check src/visualizer/document_server
```
